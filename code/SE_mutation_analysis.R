# Test whether the mutation rate in SEs correlates with
# SE enrichment in tumors vs normals
...
# load mutation data
mutSE.df <- read.table('_mutSE.bed', header = F, sep = '\t')

####################################################################
# Preprocessing
####################################################################

# define mutSE.df colnames
colnames(mutSE.df) <- c('chr', 'start', 'end')

# make mut data a genome ranges object
mutSE.GR <- makeGRangesFromDataFrame(mutSE.df,
